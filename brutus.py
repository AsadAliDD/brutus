    ...
    if rank == 0:
        ...
        for i in range(1, size):
            ...
            if result[0]:  # If the password was found
                logger.info(
                    f"Password found by worker {i}: {result[1]}, Terminating all processes."
                )
                final_result = result
                for i in range(1, size):
        ...
        if final_result[0]:
            ...
            logger.info("Finishing Execution")
            table = [["Password", "Not Found"], ["Time Taken", time_taken]]
            table_str = tabulate(table, headers=["Parameter", "Value"], tablefmt="pipe")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--chunksize",
        type=int,
        help="Size of the chunk to be processed by each process",
    )
